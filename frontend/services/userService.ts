...
export class UserService {
  private static async generateEmployeeId(
    firstName: string,
    lastName: string,
    companyCode: string = "OD"
  ): Promise<string> {
  ...
  static async createEmployee(data: {
    ...
    try {
      ...
      let companyCode: string = "OD";

      if (data.companyName) {
        const companyRes = await client.query(
          "SELECT id, code FROM companies WHERE name = $1",
          [data.companyName]
        );
        if (companyRes.rows.length === 0) {
          throw new Error(`Company '${data.companyName}' not found`);
        }
        companyId = companyRes.rows[0].id;
        companyCode = companyRes.rows[0].code;
      } else {
        // Fallback to first company if not provided (legacy behavior) or throw?
        // Assuming fallback for now as per original code, but cleaner
        const companyRes = await client.query(
          "SELECT id, code FROM companies LIMIT 1"
        );
        if (companyRes.rows.length === 0)
