...
export function TimeOffView({ isAdmin = false }: TimeOffViewProps) {
        ...
        const mapped: TimeOffRequest[] = data.map((leave: any) => ({
          ...
          name:
            leave.first_name && leave.last_name
              ? `${leave.first_name} ${leave.last_name}`
              : leave.employee_id || "Unknown",
          startDate: new Date(leave.start_date).toLocaleDateString("en-GB"),
          endDate: new Date(leave.end_date).toLocaleDateString("en-GB"),
          type: (leave.leave_type === "PAID"
            ? "Paid time Off"
            : leave.leave_type === "SICK"
            ? "Sick time off"
            : "Unpaid Leaves") as TimeOffRequest["type"],
  ...
  const handleSubmit = async () => {
    ...
    try {
      const leaveType =
        formData.timeOffType === "Paid time Off"
          ? "PAID"
          : formData.timeOffType === "Sick time off"
          ? "SICK"
          : "UNPAID";
